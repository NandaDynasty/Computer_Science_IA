...
class breadrevenue(Base):
    __tablename__ = "breadsrevenue"
...
class brmodel():
    ...
    def update(self):
        with engine.connect() as connection:
            stmt = update(breadrevenue).where(
                and_(breadrevenue.month == self.month, breadrevenue.year == self.year)).values(
                revenue=self.revenue)
            connection.execute(stmt)
            connection.commit()
    def insert(self):
        with engine.connect() as connection:
            stmt = insert(breadrevenue).values(year=self.year, month=self.month, revenue=self.revenue)
            connection.execute(stmt)
            connection.commit()
