...
class savouryrevenue(Base):
    __tablename__ = "savouryrevenue"
...
class srmodel():
    ...
    def update(self):
        with engine.connect() as connection:
            stmt = update(savouryrevenue).where(
                and_(savouryrevenue.month == self.month, savouryrevenue.year == self.year)).values(
                revenue=self.revenue)
            connection.execute(stmt)
            connection.commit()
    def insert(self):
        with engine.connect() as connection:
            stmt = insert(savouryrevenue).values(year=self.year, month=self.month, revenue=self.revenue)
            connection.execute(stmt)
            connection.commit()
